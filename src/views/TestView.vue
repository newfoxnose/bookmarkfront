  ...
  <div>
    <div v-for="item in fileitems" style="margin-bottom: 5px">
      <a
        ...
        @click="
          showDrawer(item.feed_name, item.feed_url, item.id, item.is_private)
        "
        ><form-outlined
      /></a>
  ...
  >
    <template #extra>
      <a-button
        type="primary"
        danger
        @click="deletefeed(drawer.feed_id)"
        :loading="iconLoading"
        >删除</a-button
      >
    </template>
...
export default {
  ...
  setup() {
    ...
    async function takeUrlshot() {
      try {
        let params = new URLSearchParams(); //post内容必须这样传递，不然后台获取不到
        params.append("token", $cookies.get("token"));
        params.append("timestamp", new Date().getTime());
        params.append("url", "https://mp.weixin.qq.com/s/W-ALWhvttDcR0dTbBLGcLA");
    ...
    async function takeScreenshot(content) {
      try {
