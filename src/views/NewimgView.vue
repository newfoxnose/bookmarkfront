<template>
  <h3 style="margin-top:15px;">新图片</h3>
  ...
  <div>
    <a-modal v-model:visible="visible" title="新图片创建成功" @ok="handleOk" ok-text="确认" cancel-text="取消">
      <p>跳转到图库请点击“确定”</p>
      <p>再插入新图片请点击“取消”</p>
    </a-modal>
  </div>
  ...
  setup() {
    ...
    const save = () => {
      ...
      if (proxy.$func.getVarType(valueHtml) == "undefined" || valueHtml == '') {
        ...
        proxy.$http.post('/ajax/save_imgpost_ajax/', params).then(res => {
